...
def config():
    while 1:
        try:
            least = tkinter.simpledialog.askinteger('最小整数', '最小数', initialvalue=1)
            break
        except:
            pass
    while 1:
        try:
            most = tkinter.simpledialog.askinteger('最大整数', '最大数', initialvalue=100)
            break
        except:
            pass
    # 判断输入的最小值大于最大值
    if least > most:
        ...
        start['state'] = 'normal'
        numbers.set(random.randint(least,most))
...
def starts():
    ...
    进行判断有没有设置新的最大小值
    '''
    if numbers.get() == 0:  # 没有设置执行 默认最小1最大100
        numbers.set(random.randint(1, 100))
...
def determines():
    ...
    c = number.get()
    if c == '':
        tkinter.messagebox.showerror('抱歉', '不可为空')
        return
    elif c[0] == '0':
